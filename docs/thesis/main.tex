...
予測が値動きに追従できているかを視覚的に確認する。
ただし視覚的な確認のみでは評価が主観的になるため，
予測性能の判断は次の2つの定量指標を主たる基準として行う。
...
\begin{equation}
  ...
  \label{eq:mae}
\end{equation}
を用いる。練行足は毎ステップ必ず $\pm1$ 動くため，
直前の値をそのまま予測する「変化なし」予測の MAE は常に $1$ となる。
したがって $\mathrm{MAE}<1$ は，予測が「変化なし」予測よりも誤差が小さいことの目安となる。

第二に，予測した方向と実際の方向の一致率
\begin{equation}
  \mathrm{ACC}=\frac{1}{N}\sum_{n=1}^{N}
    \mathbf{1}\!\left[\operatorname{sign}\!\left(x_n^{\ast}-x_{n-1}\right)=b_n\right]
  \label{eq:acc}
\end{equation}
を用いる。ここで $b_n=x_n-x_{n-1}\in\{-1,+1\}$ は実際の値動きの方向，
$\operatorname{sign}(\cdot)$ は符号関数，$\mathbf{1}[\cdot]$ は
括弧内の条件が成り立つとき $1$，そうでないとき $0$ をとる指示関数である
（予測が $x_n^{\ast}=x_{n-1}$ となり方向を定めない場合は不一致として数える）。
方向を当てずっぽうに予測した場合の一致率は $0.5$ であるため，
$\mathrm{ACC}>0.5$ は予測が値動きの方向の判別に寄与していることの目安となる。

MAE は予測値と実測値の隔たりの大きさを，ACC は値動きの向きを正しく
捉えられているかを表しており，両者を併せて予測性能を評価する。
...
採用した $W=10,30,60$ について，対象期間全体での MAE と ACC を
表\,\ref{tab:whole_period} に示す。

[結果の記述。例：いずれの $W$ でも MAE は基準値 $1$ を[下回り/上回り]，
ACC は $0.5$ を[上回った/下回った]。$W=[値]$ のとき最も良い予測性能を示した，など，
...
各区間の MAE と ACC を表\,\ref{tab:pattern_summary} にまとめる。
...
\begin{table}[H]\centering
  \caption{各パターン・各参照期間の予測性能（各セルは MAE / ACC）}
...
本章の結果をまとめる。全期間にわたる定量評価（表\,\ref{tab:whole_period}）では，
[全体傾向。例：MAE は「変化なし」予測の基準値 $1$ を下回り，ACC は $0.5$ を
上回ったことから，本モデルは値動きの方向の判別に一定程度寄与した]。
参照期間別には [W に関する傾向を記述]。
