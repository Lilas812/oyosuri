...
% 画像ファイルが無い場合でもコンパイルを止めないためのマクロ。
% 拡張子を省略した画像名についても pdf/png/jpg/jpeg の順に確認する。
\newcommand{\safeincludegraphics}[2][]{%
  \IfFileExists{#2}{%
    ...
    \fbox{%
      \begin{minipage}[c][45mm][c]{0.9\linewidth}
        \centering
        画像未配置\\[2mm]
        \texttt{\detokenize{#2}}
      \end{minipage}%
...
\begin{figure}[H]
  \centering
  \safeincludegraphics[width=\textwidth]{rw_transition}
...
\begin{figure}[htbp]
  \centering
  \safeincludegraphics[width=\textwidth]{rw_barchart.png}
...
\begin{theorem}[大数の法則~{\cite{refbook}}]
\label{thm:lln}
\begin{equation}
...
\begin{theorem}[中心極限定理~{\cite{refbook}}]
\label{thm:clt}
...
\begin{figure}[H]
  \centering
  \safeincludegraphics[width=0.75\textwidth]{normal_limit_distribution}
...
\begin{figure}[H]
  \centering
  \safeincludegraphics[width=\textwidth]{crw_transition}
...
\begin{figure}[htbp]
  \centering
  \safeincludegraphics[width=\textwidth]{crw_barchart}
...
中央が低く，中心から離れた位置の確率が大きい形
（裾の広い，より分散の大きい分布）になっていく。
...
\begin{proposition}[{\cite{refbook}}]
\label{prop:symmetry}
...
\begin{theorem}[相関付きランダムウォークの弱収束極限定理~{\cite{refbook}}]
\label{thm:clt_corr}
...
\begin{figure}[H]
  \centering
  \safeincludegraphics[width=0.85\textwidth]{crw_hist_n100}
...
一定の値幅ごとに上昇・下降を判定し，
$\{-1,+1\}$ の離散的な系列へ変換する。
これにより，価格変動を相関付きランダムウォークの枠組みで扱える形にする。以降，このようにして得られたデータ列を平均練行足と呼ぶ。

\subsection{データの構成}

本研究で用いる原データは，XAUUSD の価格系列である。
ここで，XAUUSD とは，金価格を米ドルで表す取引記号であり，
金1トロイオンス（約31.1\,g）当たりの米ドル価格を示す。
...
\begin{figure}[H]
  \centering
  \safeincludegraphics[width=\textwidth]{linechart_renko_compare}
...
実データに当てはめた結果を示す（\ref{subsec:full_baseline}節）。
...
用いる原データは，XAUUSD の1分ごとの始値，高値，安値，終値を記録した
1分足データである。
...
以上から，全期間参照のモデルは，MAE と HIT の両方で基準予測を下回った。
...
\begin{figure}[H]\centering
 \safeincludegraphics[width=\textwidth]{overlay_full}
 \caption{実測 $x_n$ と全期間参照による予測 $x_n^{\ast}$ の重ね描き}
 \label{fig:full_baseline_overlay}
\end{figure}

また，各時刻までの全観測を用いて評価関数を最小化したときの
推定パラメータ $p^{\ast},q^{\ast},\alpha^{\ast}$ の推移を
図\,\ref{fig:full_baseline_params} に示す。
推定値は探索格子上の値であり，最小値を与える組が複数存在する場合には，
それらの各成分の相加平均を表示している。

\begin{figure}[H]
  \centering
  \safeincludegraphics[width=\textwidth]{params_full.png}
  \caption{全期間参照における推定パラメータ
  $p^{\ast},q^{\ast},\alpha^{\ast}$ の推移}
  \label{fig:full_baseline_params}
\end{figure}

この失敗の原因は，次のように考えられる。
全期間参照による当てはめは，系列全体が単一のパラメータ $(p,q,\alpha)$ から生成され，
値動きの性質が時間によって変わらない定常な系列であると仮定することに相当する。
しかし実際の値動きは，上昇，下降，持ち合いなどの状態が
時間とともに変化する非定常な系列であり，既に終わった局面の観測が推定に混入し続けるため，
現在の局面に対する予測が系統的にずれていく
（機構の詳細は第8章で考察する）。

そこで以下では，当てはめに用いる観測を直近 $W$ 点に限る
...
そして，次時刻の予測値を，参照期間の先頭を基準とした $W$ ステップ後の期待値
\begin{equation}
  x_{n+1}^{\ast}
  =
  x_{n-W+1}
...
$W$ とともに概ね単調に増加して全期間で最大となった
（例外は $W=20$ から $W=30$ にかけての小さな逆転のみである）。
計算時間は $W$ に対して単調に増加した。
したがって，小さい参照期間が誤差・計算時間の両面で優位であり，
...
1 ステップ当たりの計算量は，おおむね $W$ の2乗に比例して増える
...
\begin{figure}[H]\centering
  \safeincludegraphics[width=\textwidth]{w_sweep_summary}
...
ここで，価格が明確な上昇または下降の傾向を示さず，
一定の値幅の中で上下を繰り返す持ち合いの状態を，
本研究ではレンジ局面とよぶ。
そこで評価の中心である図による比較として，
...
実測値の下に取り残され，
上昇から下降へ転じる高値を過ぎた局面では，
逆に上昇の傾きを引きずって実測値の上方へ乖離した。
...
\begin{figure}[H]\centering
  \safeincludegraphics[width=\textwidth]{w_overlay}
...
そこで，値動きの違いを見るため，次の4パターンを代表例として取り上げる。
...
\begin{figure}[H]\centering
  \safeincludegraphics[width=\textwidth]{pat_uptrend_w}
...
\begin{figure}[H]\centering
  \safeincludegraphics[width=\textwidth]{pat_up2range_w}
...
$W=60$ の予測は上昇から下降へ転じる高値を過ぎたあとも
長く上昇方向の予測を出し続け，
実測値から大きく乖離した（MAE $4.876$）。
$W=2$ は反転から数本で下降方向へ切り替わり，
MAE は $0.892$ と4パターン中で最小であった。
\begin{figure}[H]\centering
  \safeincludegraphics[width=\textwidth]{pat_up2down_w}
...
全期間参照による誤差は $\mathrm{MAE}=5.94$ と，
「変化なし」予測の基準値 $1$ の約 $6$ 倍に達した。
参照期間 $W$ を導入すると誤差は $W$ が小さいほど小さく，
最小の $W=2$ のとき最良（$\mathrm{MAE}=0.847$）となり，
本章では，この結果が生じた理由を本モデルの構造から検討し，
そのうえで「$W=2$ が最良」という結果の意味を批判的に整理する。
なお，方向的中率 HIT が $W$ によらずおおむね $1/2$ 前後にとどまるのは，
\ref{subsec:patterns}節で見たように，練行足の符号列そのものが方向をあまり持続させず，
次にどちらへ動くかが半々に近いためであり，$W$ の取り方ではなくデータの側の性質による。
そこで本章では，主として誤差（MAE）の側の仕組みに焦点を当てる。

\subsection{$W$ が大きいほど誤差が増える機構}\label{subsec:disc_mech}

本モデルの予測値（式\eqref{eq:rolling_prediction}）は，
参照期間の先頭 $x_{n-W+1}$ を出発点とし，
そこから $W$ 歩進んだ位置の期待値を求める構成になっている。
参照期間内の観測値は評価関数に含まれているため，
パラメータ $(p,q,\alpha)$ の推定には使われている。

しかし，推定したパラメータから期待値を求めるときには，
実際の現在位置 $x_n$ や直前の移動方向を条件としていない。
たとえば $W=60$ の場合，参照期間の先頭から現在までの $59$ 歩は
すでに観測されているが，予測では参照期間の先頭から $60$ 歩進んだ位置の
期待値を改めて計算している。
したがって，現在位置や直前の方向はパラメータ推定を通して間接的には影響するが，
期待値の計算には直接使われていない。

$W$ が大きいほど予測の出発点は現在位置から遠くなり，
期待値を求める歩数も多くなる。
このことが，$W$ の増加とともに MAE が大きくなった一因であると考えられる
（表\,\ref{tab:w_sweep}）。

\subsection{局面の混在と転換への追従の遅れ}\label{subsec:disc_mix}

参照期間 $W$ が大きいと，上昇・レンジといった複数の局面が同時に含まれ得る。
このとき参照期間全体から推定されるパラメータは，
いずれの局面にも十分には適合しない，いわば妥協点のような値になる。
加えて，局面が転換したあとも，転換前の局面に属する足が参照期間から抜け切るまでに
...
\ref{subsec:disc_mech}節で述べた，遠い起点から長い区間を見積もることによる乖離を受けにくい。
その一方でこの設定は，参照期間が直近の $1$ 本しか含まないために
過去の値動きを参照する効果をほとんど利用していない。
すなわち $W=2$ が最良であったことは，モデルが局面を捉えた成果ではなく，
履歴を参照するほど誤差を上乗せする本モデルの構造のもとで，
参照量を最も絞ることで悪化を最小にとどめた結果と見るべきである。

なお，この短い $W$ の優位も普遍ではなく，
方向の持続が乏しいレンジ主体の局面では失われる（\ref{subsec:patterns}節）。
小さい $W$ では予測値と実測値の乖離が比較的小さく，
特に $W=2$ では「変化なし」予測を下回る MAE が得られた。
一方，HIT はおおむね $0.5$ 前後であり，
次の値動きの方向を安定して予測できたとはいえない。
\newpage
\section{今後の課題}
...
\begin{itemize}
  \item \textbf{現在位置・直前の方向を条件とする予測への拡張}：
        観測経路は評価関数によるパラメータ推定には用いられているが，
        推定後の期待値は現在位置や直前の方向で条件づけられていない。
        時刻 $n$ の位置と向きを条件とした条件付き期待値
        $E(\tilde{S}_{n+1}\mid \tilde{S}_n=x_n,\ \text{直前の向き})$
        を予測値に用いる定式化に改めれば，
        参照期間を延ばしたときの劣化を抑えられる可能性がある。

  \item \textbf{他の金融資産への適用}：
        本研究で扱った対象は金価格（XAUUSD）の一つの時間帯に限られている。
        株価，為替，暗号資産，金以外の商品などにも同じ手法を適用し，
        資産ごとの値動きの性質によって予測結果がどのように変化するかを
        比較する必要がある。

  \item \textbf{平均練行足を用いないデータへの拡張}：
        本研究では，価格系列を平均練行足によって増分 $\pm1$ の系列に変換した。
        原価格や一定時間ごとの価格変化に直接適用するためには，
        移動幅が一定でない系列を扱えるようにモデルが取り得る値の範囲や評価関数を拡張する，
        または平均練行足とは異なる離散化方法を導入する必要がある。
        これにより，本研究の結果が平均練行足への変換にどの程度依存するかも検証できる。

  \item \textbf{他の確率モデルとの比較}：
        同じデータに対して，現在の状態から次の状態への移り方を確率で表すマルコフ連鎖，
        直前の1本だけでなく，複数本前までの値動きを考慮したモデル，
        局面の切り替わりを表現するモデルなどを適用し，
        MAE，HITおよび計算時間を本モデルと比較する必要がある。
        その比較により，相関付きランダムウォークを用いる利点と限界を
        より明確にできると考えられる。
...
本研究を進めるにあたり，終始丁寧なご指導をいただきました
愛媛大学工学部工学科応用数理研究室の安藤和典教授，
森岡悠准教授に心より感謝申し上げます。
また，本論文の審査をお引き受けいただきました
乗松真二准教授，宇戸寿幸准教授に深く感謝申し上げます。
...
\bibitem{refbook}
  今野紀雄，『量子ウォークによる時系列解析』，日本評論社（2020）．

\end{thebibliography}
\newpage
\appendix
\section{ボックス幅別の全期間参照結果}
\label{app:full_by_B}

本編では，対象区間の値動きの特徴を失わないように
ボックス幅を $B=3$ ドルと定めた。
ここでは補足資料として，ボックス幅を
$B=2,3,4,5$ ドルと変えた場合について，
同一期間の平均練行足を生成し，
全期間参照による予測を行った結果を示す。

対象としたのは，2026年6月15日7時01分から
6月16日5時57分まで（日本時間）である。
元の1分足データは1377本であり，
各ボックス幅から得られた練行足の本数は
表\ref{tab:app_full_summary}のとおりである。

\begin{table}[H]
  \centering
  \caption{ボックス幅別の全期間参照結果}
  \label{tab:app_full_summary}
  \begin{tabular}{c|r|c|c}
    \hline
    ボックス幅 $B$ & 練行足本数 $N$ & MAE & HIT \\
    \hline
    2 & 772 & 8.1349 & 0.4786 \\
    3 & 365 & 4.6812 & 0.5110 \\
    4 & 202 & 3.1845 & 0.4677 \\
...
ボックス幅が小さいほど，同じ価格変動から生成される練行足の本数は多くなる。
以下では，各ボックス幅について，
実測値と全期間参照による予測値の重ね描きと，
各時点で推定されたパラメータ
$p^{\ast},q^{\ast},\alpha^{\ast}$を示す。

\subsection{$B=2$ ドル}

\begin{figure}[H]
  \centering
  \safeincludegraphics[width=\textwidth]
    {overlay_2026-06-15_B2_full.png}

  \vspace{5mm}

  \safeincludegraphics[width=\textwidth]
    {params_2026-06-15_B2_full.png}

  \caption{$B=2$ ドルにおける全期間参照の結果。
  上：実測値と予測値，下：推定パラメータ}
  \label{fig:app_full_b2}
\end{figure}
...
\begin{figure}[H]
  \centering
  \safeincludegraphics[width=\textwidth]
    {overlay_2026-06-15_B3_full.png}

  \vspace{5mm}

  \safeincludegraphics[width=\textwidth]
    {params_2026-06-15_B3_full.png}

  \caption{$B=3$ ドルにおける全期間参照の結果。
  上：実測値と予測値，下：推定パラメータ}
  \label{fig:app_full_b3}
\end{figure}
...
\begin{figure}[H]
  \centering
  \safeincludegraphics[width=\textwidth]
    {overlay_2026-06-15_B4_full.png}

  \vspace{5mm}

  \safeincludegraphics[width=\textwidth]
    {params_2026-06-15_B4_full.png}

  \caption{$B=4$ ドルにおける全期間参照の結果。
  上：実測値と予測値，下：推定パラメータ}
  \label{fig:app_full_b4}
\end{figure}
...
\begin{figure}[H]
  \centering
  \safeincludegraphics[width=\textwidth]
    {overlay_2026-06-15_B5_full.png}

  \vspace{5mm}

  \safeincludegraphics[width=\textwidth]
    {params_2026-06-15_B5_full.png}

  \caption{$B=5$ ドルにおける全期間参照の結果。
  上：実測値と予測値，下：推定パラメータ}
  \label{fig:app_full_b5}
\end{figure}

\section{$B=2$ における150ステップごとの結果}
\label{app:b2_chunk150}

同じ期間から得られた $B=2$ ドルの練行足772本に対して，
参照期間を $W=10,30,60$ とした予測を行った。
予測自体は1日分の系列に対して連続して計算し，
結果を比較しやすくするため，図では150ステップごとの区間に分けて表示した。
最後の $n=751$ から $772$ までの区間は22ステップしかなく，
他の区間と長さが異なるため，ここでは掲載しない。
各区間の MAE と HIT を表\ref{tab:app_b2_chunk150}に示す。

\begin{table}[H]
  \centering
  \small
  \caption{$B=2$ における150ステップごとの MAE と HIT}
  \label{tab:app_b2_chunk150}
  \begin{tabular}{c|c|ccc|ccc}
    \hline
    & & \multicolumn{3}{c|}{MAE} & \multicolumn{3}{c}{HIT} \\
    区間 & ステップ $n$ & $W=10$ & $W=30$ & $W=60$
      & $W=10$ & $W=30$ & $W=60$ \\
    \hline
    1 & 1--150   & 1.4570 & 2.5487 & 3.6464 & 0.4765 & 0.4295 & 0.3960 \\
    2 & 151--300 & 1.4569 & 1.7235 & 2.7351 & 0.4899 & 0.5302 & 0.4832 \\
    3 & 301--450 & 1.0659 & 1.6581 & 3.3320 & 0.5906 & 0.5235 & 0.4497 \\
    4 & 451--600 & 1.2012 & 1.5630 & 2.8382 & 0.5503 & 0.5503 & 0.4564 \\
    5 & 601--750 & 1.3992 & 1.5826 & 2.4881 & 0.4899 & 0.5436 & 0.4765 \\
    \hline
  \end{tabular}
\end{table}

MAE は5区間すべてで $W=10$ のときに最も小さかった。
一方，HIT は区間によって最も高い $W$ が異なり，
参照期間を変えても一貫した傾向は見られなかった。
以下では，各区間を1ページずつ用い，
実測値と予測値の重ね描き，$W=10,30,60$ の推定パラメータの順に，
4枚の図を上から縦に示す。

% #1：表示するステップ範囲
% #2：画像ファイル名に用いるステップ範囲
% #3：図のラベルに用いる区間番号
\newcommand{\chunkresultpage}[3]{%
  \clearpage
  \begin{figure}[H]
    \centering

    \par\noindent
    \makebox[\textwidth][c]{実測値と予測値（$n=#1$）}
    \par\vspace{1mm}
    \safeincludegraphics[
      width=0.98\textwidth,
      height=0.19\textheight,
      keepaspectratio
    ]{overlay_B2_#2_W10_W30_W60.png}

    \par\vspace{2mm}
    \makebox[\textwidth][c]{推定パラメータ（$W=10$）}
    \par\vspace{1mm}
    \safeincludegraphics[
      width=0.98\textwidth,
      height=0.18\textheight,
      keepaspectratio
    ]{params_B2_#2_W10.png}

    \par\vspace{2mm}
    \makebox[\textwidth][c]{推定パラメータ（$W=30$）}
    \par\vspace{1mm}
    \safeincludegraphics[
      width=0.98\textwidth,
      height=0.18\textheight,
      keepaspectratio
    ]{params_B2_#2_W30.png}

    \par\vspace{2mm}
    \makebox[\textwidth][c]{推定パラメータ（$W=60$）}
    \par\vspace{1mm}
    \safeincludegraphics[
      width=0.98\textwidth,
      height=0.18\textheight,
      keepaspectratio
    ]{params_B2_#2_W60.png}

    \caption{$B=2$，$n=#1$ における予測結果と推定パラメータ}
    \label{fig:app_b2_chunk#3}
  \end{figure}
}
\chunkresultpage{1-150}{steps_0001_0150}{1}
\chunkresultpage{151-300}{steps_0151_0300}{2}
\chunkresultpage{301-450}{steps_0301_0450}{3}
\chunkresultpage{451-600}{steps_0451_0600}{4}
\chunkresultpage{601-750}{steps_0601_0750}{5}
