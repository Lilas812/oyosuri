...
\begin{enumerate}
  ...
  \item 特に $a = 1/2$（すなわち $p = a = 1/2$）のときは，初期確率ベクトル $\varphi$ によらず
        （初期条件を問わず），任意の $n \in \mathbb{Z}_{\geq 0}$ と $x \in \mathbb{Z}$ に対して
        $P(\tilde{S}_n = x) = P(\tilde{S}_n = -x)$ が成立する。
\end{enumerate}
\end{proposition}

(2) の場合を確認する。$a = 1/2$ のとき，コイン行列は
\[
...
各図には，対称な通常のランダムウォークの極限分布である標準正規分布
$N(0,1)$（破線，系\,\ref{cor:clt_sym}）と，
定理\,\ref{thm:clt_corr} の極限分布 $N\!\left(0,\,a/(1-a)\right)$（実線）を
重ねて描いた。

持続性をもつ場合として $a = 2/3$（$\sigma^2 = 2$）の結果を
図\,\ref{fig:crw_limit_persistent} に，
反転傾向をもつ場合として $a = 1/3$（$\sigma^2 = 1/2$）の結果を
図\,\ref{fig:crw_limit_antipersistent} に示す。
さらに，両者の分布を標準正規分布とともに 1 つの図に重ねた比較を
図\,\ref{fig:crw_hist_n100} に示す。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{crw_limit_persistent}
  \caption{持続性をもつ相関付きランダムウォーク（$a=2/3$）の時刻 $n=100$ における $\tilde{S}_n/\sqrt{n}$ の分布と正規分布の比較}
  \label{fig:crw_limit_persistent}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{crw_limit_antipersistent}
  \caption{反転傾向をもつ相関付きランダムウォーク（$a=1/3$）の時刻 $n=100$ における $\tilde{S}_n/\sqrt{n}$ の分布と正規分布の比較}
...
図\,\ref{fig:crw_limit_persistent}〜図\,\ref{fig:crw_hist_n100} から，
次のことが読み取れる。

第一に，時刻 $n=100$ における $\tilde{S}_n/\sqrt{n}$ のヒストグラムは，
いずれの場合も極限分布 $N(0,\,a/(1-a))$ の密度曲線（実線）とほぼ重なっている。
実際，$\tilde{S}_n/\sqrt{n}$ の分散を数値計算すると，
$a=2/3$ のとき $1.99$（極限値 $2$），$a=1/3$ のとき $0.50$（極限値 $1/2$）であり，
定理\,\ref{thm:clt_corr} の弱収束は $n=100$ 程度の有限時刻で
既に良い近似として成立していることが確認できる。
...
第三に，極限分散は $r = 2a-1$ を用いて
\[
  \sigma^2 = \frac{a}{1-a} = \frac{1+r}{1-r}
\]
と書き直せる。ここで $r = a - (1-a)$ は，直前と同じ方向へ進む確率と
反対方向へ進む確率の差であり，連続する 2 ステップの方向の相関の強さを表す。
$r>0$（持続性）ならば $\sigma^2>1$，$r<0$（反転傾向）ならば $\sigma^2<1$，
$r=0$ ならば $\sigma^2=1$ となる。
すなわち，各ステップの移動量が $\pm1$ である点は通常のランダムウォークと
同じであっても，直前の方向という 1 ステップ分の記憶は，
長時間スケールでは分布のばらつきの大きさの違いとして現れる。
一方で，極限が正規分布になるという性質自体は相関によって変わらない。

この性質は，次章以降で扱う金価格の値動きの解析においても基本となる。
値動きの系列に持続性があれば，同じ本数の足が経過したときの累積的な
値動きの広がりは，独立な値動き（通常のランダムウォーク）を仮定した場合より
大きくなり，反転傾向があれば小さくなる。
したがって，観測された値動きの系列から持続性・反転傾向の強さ
（本章のパラメータ $a$，より一般には次章の $p,q$）を推定することが，
本研究で構成する時系列モデルの中心的な役割となる。
...
%  \includegraphics[width=\textwidth]{full_baseline_overlay}
...
\begin{table}[H]\centering
  \caption{各パターン・各参照期間の MAE（合成データ，$N=150$）}
  \label{tab:pattern_summary}
  \begin{tabular}{l|ccc}\hline
    パターン & $W=2$ & $W=10$ & $W=60$ \\\hline
    上昇継続     & 0.996 & 1.407 & 4.247 \\
    レンジ→上昇 & 1.082 & 1.124 & 2.442 \\
    上昇→レンジ & 0.996 & 1.368 & 2.963 \\
    上昇→下降   & 0.892 & 1.485 & 4.876 \\\hline
  \end{tabular}
\end{table}
...
本章の結果をまとめる。
まず，第5章のモデルをそのまま全期間参照で実データに当てはめたベースラインは，
$\mathrm{MAE}=5.94$・$\mathrm{HIT}=0.43$ と，
「変化なし」予測（$\mathrm{MAE}=1$）・無作為な方向予測（$\mathrm{HIT}=0.5$）の
いずれの基準も下回り，そのままでは妥当性が低いことが示された（\ref{subsec:full_baseline}節）。
この失敗を受けて導入した参照期間 $W$ に対する
感度分析（表\,\ref{tab:w_sweep}・図\,\ref{fig:w_sweep}）では，
小さい $W$ が誤差・計算時間の両面で優位であり，
MAE・計算時間とも $W$ を大きくするほど概ね増加した。
すなわち，精度と計算量のあいだのトレードオフは存在せず，
本モデルを予測に用いる限り参照期間は短くとるのがよい，という結果である。
ただし，「変化なし」予測の基準値 $1$ を下回るのは $W=2,3$ に限られ，
参照期間の導入による改良の効果は直近数本を参照する範囲にとどまった。

重ね描きによる視認比較（図\,\ref{fig:w_overlay}）はこの傾向と整合しており，
小さい $W$ の予測は値動きの継続にも転換にも短い遅れで追従する一方，
大きい $W$ の予測は転換のあとも古い方向を引きずって実測値から長く乖離した。

局面別の分析（表\,\ref{tab:pattern_summary}）では，
4パターンのすべてで MAE が $W$ について単調に増加した。
差が最も大きいのは上昇→下降であり，
上昇継続でも押しを挟む箇所で大きい $W$ の上振れが蓄積した。
一方，方向の持続が乏しいレンジ主体の局面では小さい $W$ の優位は縮み，
$W=2$ と $W=10$ がほぼ同等となった。
したがって，感度分析で観測された「$W$ が大きいほど誤差が大きい」という単調な傾向は，
主として経路が曲がる箇所（転換や押し）における追従の遅れが
$W$ とともに長引くことに由来すると整理できる。
この結果がモデルのどのような構造から生じるのかは，次章で考察する。

\newpage
\section{考察}

前章の実験では，全期間参照によるベースラインが，
「変化なし」予測や無作為な方向予測といった単純な基準にも及ばない一方，
参照期間 $W$ が小さいほど予測誤差も計算時間も小さいという結果が得られ，
精度と計算量の間に「一方を良くすればもう一方が悪くなる」という関係は見られなかった。
本章では，これらの結果が本モデルのどのような構造から生じるのかを検討し，
そのうえで本モデルの妥当性と限界を整理する。

\subsection{参照期間依存性が生じる機構}

まず，誤差が $W$ とともに増える理由を予測値の作り方から考える。
参照期間 $W$ の予測値（式\eqref{eq:rolling_prediction}）は，
窓の先頭 $x_{n-W+1}$ を出発点として，そこから $W$ 歩進んだ先の平均的な到達点を答える形になっている。
ここで注意したいのは，この $W$ 歩のうち手前の $W-1$ 歩は，
すでに $x_{n-W+1}\to\cdots\to x_n$ という実際の値動きとして観測済みだという点である。
本来なら残る 1 歩，すなわち現在位置 $x_n$ から次の $x_{n+1}$ への 1 歩だけを見積もればよい。
ところが本モデルは，この観測済みの途中経過を予測に使わず，
$W$ 歩ぶんをまとめて確率的な平均で置き換えてしまう。
その結果，予測には，いま値がどこにあるか（現在位置 $x_n$）も，
直前がどちら向きに動いたかも，直接は反映されない。
言いかえれば，予測は「いまの状況を手がかりにしない平均値」になっている。

ここで，そもそもこの「平均的な到達点」がどのように出てくるのかを改めて確認しておく。
本モデルは，まず窓内の $W$ 点にいちばんよく合うパラメータ $(p,q,\alpha)$ を，
評価関数 $V$ の最小化によって選ぶ（式\eqref{eq:model_Vn}）。
こうして選ばれたパラメータをもつ相関付きランダムウォークは，
第4章の時間発展（式\eqref{eq:crw_evolution}）を $t$ 回くり返すことで，
「$t$ 歩後に位置 $x$ にいる確率」$\mu_t(x;p,q,\alpha)$（式\eqref{eq:model_mu}）を与える。
予測値は，この確率分布 $\mu_W$ の\textbf{平均（重心）}を出発点 $x_{n-W+1}$ に足したものである。
実際の値動きは毎回この平均のまわりに散らばるので，
平均から外れた分がそのまま予測誤差になる。

評価関数の構造も，この「平均からのずれ」と直接つながっている。
式\eqref{eq:model_Vn} の内側の和は，平均のまわりに次のように分解できる。
\begin{equation}
  ...
  \label{eq:Vn_decomp}
\end{equation}
第 1 項はモデルの分布そのものの散らばりの大きさ（分散）で，
観測値によらず時刻 $t$ とともに増える。
そのため最小化は，散らばりの小さい，すなわちできるだけ動きの定まったパラメータを選ぶ方向に働く。
第 2 項は，モデルの平均が実際の観測値からどれだけ離れているかを表す項で，
遅い時刻ほど重みが大きく（傾きのわずかなずれ $\delta$ も時刻が経つと $(\delta t)^2$ の形で大きく効く），
当てはめは窓の後ろ寄りの観測に平均経路を沿わせる。
結果として，参照期間 $W$ の予測は，
窓内の $W$ 点を 1 本の直線（一定の傾き）で近似し，
その直線をそのまま先へ延ばして次の点を当てる，という振る舞いに近くなる。

このとき予測誤差は，窓内の実際の経路が 1 本の直線からどれだけ外れているかで決まり，
その外れは $W$ とともに大きくなる。
第一に，方向の定まらない区間では，先を見るほど実際の位置は平均から大きく散らばる。
その散らばりの幅は，見る歩数のおよそ平方根に比例して増える
（第4章の定理\,\ref{thm:clt_corr} が示す性質）。
第二に，窓の途中に局面の転換が入ると，転換前の情報がおよそ $W$ 本ぶん参照され続けるため，
新しい局面に追いつくまでの遅れが $W$ に比例して長引く。
\ref{subsec:patterns}節で，転換を含む局面ほど $W$ 間の差が大きかったことは，
この第二の機構に対応する。
全期間参照では基準が常に $x_0$ であり，
局面をまたいだずれが解消されずに蓄積するため，誤差は最も大きくなる。
このように，「$W$ が小さいほど誤差が小さい」という実験結果は，
たまたまそうなったのではなく，いまの状況を手がかりにしない平均値で予測するという
本モデルの作りそのものから生じていると考えられる。

\subsection{練行足系列の記憶の長さ}

データの側からも同じ結論が支持される。
本モデルがもつ記憶は短く，次の方向に効くのは直前の 1 本だけである。
一方，実験に用いた符号列では，方向が直前と逆になる割合（反転率）が $\hat{r}=0.381$，
同じ方向が続く本数は平均 $2.6$ 本であった。
つまり同じ方向が続くのはせいぜい数本で，
それより前の足は現在の方向とほとんど関係をもたない。
したがって参照期間を延ばしても新しく役立つ情報はほとんど増えず，
むしろ既に終わった局面の足が推定に混じり込んで，
いまの局面に合わせたパラメータ推定をぼやけさせてしまう
（参照期間を長くとることは，値動きの性質がずっと一定であると仮定するのに近いが，
実際の値動きは上昇・下降・持ち合いと局面が入れ替わっていく）。
この意味でも，短い参照期間が有利という結果は，
練行足系列の記憶の短さと合っている。
...
この形から，$W=2$ における本モデルの MAE は，一つの式として直接書き下せる。
...
\textbf{限界}について。
第一に，予測がいまの状況を手がかりにしない平均値で作られるため，
...
\begin{itemize}
  \item \textbf{現在位置・直前の方向を条件とする予測への拡張}：
        本モデルの誤差の主因は，予測がいまの状況を手がかりにしない平均値で作られる点にあった。
