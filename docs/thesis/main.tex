...
本実験で調整するパラメータは，予測に用いる直近の観測点の個数（参照期間）$W$ のみである。
以下ではまず実験の設定を述べ，次に $W$ を系統的に変えたときの
予測誤差と計算時間の変化を主実験として調べる。

\subsubsection*{データ}

% ★仮置き：以下の図表のうち w_sweep_summary / w_overlay と tab:w_sweep の数値は，
% 実データ CSV 入手までの間，合成データ（複数局面の合成符号列，N=200）で生成した
% 暫定値である。実データ入手後に colab/oyosuri_experiment.py の run_w_sweep /
% run_multi_w_overlay を CSV に対して実行し，本節の [ ] と合わせて差し替えること。
用いる原データは XAUUSD の1分足価格系列 $P_0,P_1,\ldots,P_T$ であり，
[対象期間（例：20XX年X月X日〜同年X月X日）]% ← 実際の期間に置き換え
の区間を対象とする。
前章で述べた手順に従って値幅 $B=[値]$% ← 実際の値に置き換え
の平均練行足を構成し，符号列 $b_1,\ldots,b_N$（足の総数 $N=[値]$ 本）を得た。
これを累積した整数時系列 $x_n=\sum_{k=1}^{n}b_k$（$x_0=0$）を解析の入力とする。

値幅 $B$ は，[選定方針を記述。例：生成される足の本数が解析に十分であり，
かつ微小な価格変動を過度に足として拾わない値となるよう，予備的に複数の候補を
比較して定めた]。

得られた符号列の基本的な性質として，$+1$（上昇）の比率は $0.56$，% ← 実データで置き換え（現値は合成データ）
直前と逆符号になった遷移の比率（以下，\textbf{反転率} $\hat{r}$）は $\hat{r}=0.46$，% ← 実データで置き換え（現値は合成データ）
同符号が連続する本数（連長）の平均は $2.2$ 本であった。% ← 実データで置き換え（現値は合成データ）
これらの統計量は，考察において系列の記憶の長さを論じる際に参照する。
...
また，参照期間を区切らず $D_n$ 全体を用いる場合を\textbf{全期間}とよぶ。
全期間は第5章のモデルそのもの（式\eqref{eq:model_prediction}，基準は $x_0=0$）に対応する。

評価関数の最小化は，$p,q,\alpha\in\{0,\,0.1,\,0.2,\ldots,1\}$ の格子
...
結果はほぼ単調であった。
すなわち，MAE は最小クラスの $W=2,3$ で最も小さく，% ← 実データの結果に合わせて表現を調整
それより大きい $W$ では $W$ とともに単調に増加して全期間で最大となった。
計算時間は $W$ に対して単調に増加した。
したがって，\textbf{小さい参照期間が誤差・計算時間の両面で優位であり，
精度と計算量のあいだのトレードオフは観測されなかった}。
MAE が「変化なし」予測の基準値 $1$ を下回るのは $W=3$ 以下の範囲であった。% ← 実データで置き換え
...
% ★仮置き（合成データ N=200 による暫定値）：実データ入手後に
% colab/oyosuri_experiment.py の run_w_sweep の出力（w_sweep_to_latex）で差し替える。
\begin{table}[H]\centering
  \caption{参照期間 $W$ ごとの予測誤差と計算時間}
  \label{tab:w_sweep}
  \begin{tabular}{l|cccccccc}\hline
    指標 & $W=2$ & $W=3$ & $W=5$ & $W=10$ & $W=20$ & $W=30$ & $W=60$ & 全期間 \\\hline
    MAE & 0.945 & 0.913 & 1.271 & 1.366 & 1.599 & 1.777 & 3.431 & 6.333 \\
    総計算時間 [s] & 8.1 & 12.6 & 16.3 & 30.5 & 59.5 & 89.1 & 195.2 & 511.2 \\
    1ステップ当たり [ms] & 41 & 63 & 82 & 152 & 298 & 446 & 976 & 2556 \\\hline
  \end{tabular}
\end{table}

% ★仮置き（合成データによる暫定図）：実データ入手後に差し替える。
...
\footnote{序盤の $n<W-1$ では全履歴を参照するため（\ref{subsec:exp_setup}節の予測手順），
...
本章の結果をまとめる。
参照期間 $W$ に対する感度分析（表\,\ref{tab:w_sweep}・図\,\ref{fig:w_sweep}）では，
小さい $W$ が誤差・計算時間の両面で優位であり，
MAE・計算時間とも $W$ を大きくするほど増加した。
...
前章の実験では，参照期間 $W$ が小さいほど予測誤差も計算時間も小さいという
結果が得られ，精度と計算量のトレードオフは観測されなかった。
...
一方，実験に用いた符号列では反転率が $\hat{r}=0.46$，% ← 実データで置き換え（現値は合成データ）
同符号の連長は平均 $2.2$ 本であった。% ← 実データで置き換え（現値は合成データ）
...
\[
  ...
  \qquad (0<\gamma<1)
\]
の形になる（本実験の格子では $\gamma=0.645$ であり，
実際に予測列は全ステップで $x_{n+1}^{\ast}=x_n+0.645\,b_n$ に一致した）。
これは「直前と同じ方向へ動く」と読む\textbf{順張り予測}を，
複数解の平均によってやや縮小したものにほかならない。

順張り予測そのもの（$x_{n+1}^{\ast}=x_n+b_n$）の MAE は，
継続で $0$，反転で $2$ の誤差を出すから $2\hat{r}$ である。
実験系列では $2\hat{r}=0.91$ に対し，本モデルの最良値は
$\mathrm{MAE}=0.913$（$W=3$，$W=2$ では $0.945$）であった。% ← 実データで置き換え（現値は合成データ）
すなわち，誤差が最小となる設定における本モデルは，
単純な順張り予測とほぼ同等の予測を与えており，
MAE の意味でそれを明確に上回るわけではない。
...
第二に，その帰結として転換局面に弱く，
誤差最小の設定での振る舞いは単純な順張り予測とほぼ同等にとどまる。
第三に，本研究の結果には次の留保が付く。
