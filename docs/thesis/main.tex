...
XAUUSD の実価格データに適用し，その予測性能を定量的に評価する。
...
時刻 $n$ における予測は，直近 $W$ 本（参照期間）の部分系列のみを用いて
評価関数 $V_n(p,q,\alpha)$（式\eqref{eq:model_Vn}）を最小化し，
得られたパラメータのもとで式\eqref{eq:model_prediction} の期待値を
次時刻の予測値 $x_{n+1}^{\ast}$ とすることで行う。
...
\subsection{予備実験：参照期間の検討}

参照期間 $W$ は予測性能を左右するパラメータである。
まず $W$ を極端にとった場合として，$W=2$ と全期間
（参照期間を区切らず時刻 $n$ までのすべての足を用いる場合）を比較する。
両者の予測性能を表\,\ref{tab:prelim_w} に，予測の様子を図\,\ref{fig:prelim_w} に示す。

[結果の記述。例：$W=2$ では直近の値動きのみを参照するため予測が過敏に振れ，
全期間では古い情報の影響が残って転換への追従が遅れる傾向がみられた。]

\begin{table}[H]\centering
  \caption{極端な参照期間における全期間の予測性能}
  \label{tab:prelim_w}
...
\begin{figure}[H]\centering
  \includegraphics[width=\textwidth]{prelim_w}
  \caption{極端な参照期間（$W=2$・全期間）における実測 $x_n$ と予測 $x_n^{\ast}$}
  \label{fig:prelim_w}
\end{figure}

以上より，極端な値を避けた中庸な参照期間として $W=10,30,60$ の3通りを採用し，
以降の解析に用いる。

\subsection{全期間における予測性能}

採用した $W=10,30,60$ について，対象期間全体での MAE を
表\,\ref{tab:whole_period} に示す。

[結果の記述。例：いずれの $W$ でも MAE は基準値 $1$ を[下回り/上回り]，
$W=[値]$ のとき最も良い予測性能を示した，など，
まず本モデル全体の予測性能の傾向を述べる。]

\begin{table}[H]\centering
  \caption{全期間における各参照期間の予測性能}
  \label{tab:whole_period}
  \begin{tabular}{l|ccc}\hline
    指標 & $W=10$ & $W=30$ & $W=60$ \\\hline
    MAE & ? & ? & ? \\\hline
  \end{tabular}
\end{table}

% 全期間の重ね描きは点数が多く密になりやすい。
% 全体が見にくい場合は代表的な区間の抜粋に差し替えるか，図ごと省いて表だけでもよい。
\begin{figure}[H]\centering
  \includegraphics[width=\textwidth]{whole_period_w}
  \caption{全期間における実測 $x_n$ と $W=10,30,60$ の予測}
  \label{fig:whole_period}
\end{figure}

\subsection{チャートパターン別の分析}

前節の全期間にわたる結果を踏まえ，本節では値動きの局面ごとに予測の振る舞いを調べる。
本モデルは値動きの上昇方向と下降方向を対称に扱うため，
下降局面における予測は対応する上昇局面の予測を上下反転したものとなる。
そこで上昇局面のライフサイクルに対応する次の4パターンを代表例として取り上げる。

[パターンの抽出基準を記述。例：整数時系列 $x_n$ の一定区間における変化量と変動幅に
基づき，上昇が継続する区間，レンジから上昇へ移る区間，上昇からレンジへ移る区間，
上昇から下降へ転じる区間をそれぞれ抽出した。]

各パターンについて $W=10,30,60$ の予測を実測値と同一図に重ねて示し，
各区間の MAE を表\,\ref{tab:pattern_summary} にまとめる。

\subsubsection*{上昇継続}
図\,\ref{fig:pat_uptrend} に示す。
[結果の記述。例：上昇が継続する局面では，直前と同じ方向へ進みやすいという
持続性をモデルが捉え，予測が実測値に概ね追従した。]
\begin{figure}[H]\centering
  \includegraphics[width=\textwidth]{pat_uptrend_w}
  \caption{上昇継続：実測 $x_n$ と $W=10,30,60$ の予測}
  \label{fig:pat_uptrend}
\end{figure}

\subsubsection*{レンジ→上昇}
図\,\ref{fig:pat_range2up} に示す。
[結果の記述。例：レンジから上昇へ移行する局面で，
ブレイク直後の転換に予測が追従[できた/遅れた]。]
\begin{figure}[H]\centering
  \includegraphics[width=\textwidth]{pat_range2up_w}
  \caption{レンジ→上昇：実測 $x_n$ と $W=10,30,60$ の予測}
  \label{fig:pat_range2up}
\end{figure}

\subsubsection*{上昇→レンジ}
図\,\ref{fig:pat_up2range} に示す。
[結果の記述。例：上昇からレンジへ移る局面で，天井付近で上昇の勢いを
引きずり予測が[上振れした/速やかに追従した]。]
\begin{figure}[H]\centering
  \includegraphics[width=\textwidth]{pat_up2range_w}
  \caption{上昇→レンジ：実測 $x_n$ と $W=10,30,60$ の予測}
  \label{fig:pat_up2range}
\end{figure}

\subsubsection*{上昇→下降}
図\,\ref{fig:pat_up2down} に示す。
[結果の記述。例：上昇から下降へ転じる局面では，直前までの上昇の持続性を
仮定するモデルの性質上，反転に追従できず予測が大きく外れた。]
\begin{figure}[H]\centering
  \includegraphics[width=\textwidth]{pat_up2down_w}
  \caption{上昇→下降：実測 $x_n$ と $W=10,30,60$ の予測}
  \label{fig:pat_up2down}
\end{figure}

\begin{table}[H]\centering
  \caption{各パターン・各参照期間の MAE}
  \label{tab:pattern_summary}
  \begin{tabular}{l|ccc}\hline
    パターン & $W=10$ & $W=30$ & $W=60$ \\\hline
    上昇継続     & ? & ? & ? \\
    レンジ→上昇 & ? & ? & ? \\
    上昇→レンジ & ? & ? & ? \\
    上昇→下降   & ? & ? & ? \\\hline
...
本章の結果をまとめる。全期間の評価（表\,\ref{tab:whole_period}）では，
[全体傾向。例：MAE は「変化なし」予測の基準値 $1$ を[下回り/上回り]，
図からは予測が値動きに概ね追従する様子が確認できた，など]。
参照期間別には [W に関する傾向を記述]。

局面別の分析（表\,\ref{tab:pattern_summary}）では，
上昇が継続する局面で予測が値動きに追従する一方，
転換点（レンジ→上昇・上昇→レンジ・上昇→下降）では追従が遅れる，
あるいは反転に追従できず予測を外す傾向が確認された。
これは，直前と同じ方向へ進みやすいという持続性を仮定する本モデルの性質に起因すると
考えられ，持続的な局面に強く，転換点に弱いという特徴を示している。
