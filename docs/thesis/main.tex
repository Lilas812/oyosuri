...
一定の値幅ごとに上昇・下降を判定し，
$\{-1,+1\}$ の離散的な系列へ変換する。
これにより，価格変動を相関付きランダムウォークの枠組みで扱える形にする。以降,このようにして得られたデータ列を平均連行足と呼ぶ。
...
この順序は，局面別の合成データを（乱数の種を変えて）作り直しても保たれる，
安定して現れる傾向であった。
...
そこから $W$ 歩進んだ先の「平均的な到達点」を答える，という構成になっている。
ここで平均的な到達点とは，次のようにして出てくる値である。
まずモデルは，参照期間内の値動きにいちばんよく合うパラメータ $(p,q,\alpha)$ を
評価関数の最小化によって選ぶ。
そのパラメータをもつ相関付きランダムウォークは，
$t$ 歩後に位置 $x$ にいる確率 $\mu_t(x)$（式\eqref{eq:model_mu}）を与えるので，
予測値はこの確率分布の平均（重心）を出発点 $x_{n-W+1}$ に足したものになる。

問題は，この見積もりが出発点から $W$ 歩ぶんもある点である。
たとえば $W=60$ であれば，$60$ 本前の地点を起点として $60$ 歩先を見積もることになるが，
そのうち手前の $59$ 歩は，すでに $x_{n-W+1}\to\cdots\to x_n$ という
実際の値動きとして観測済みである。
本来なら残る $1$ 歩，すなわち現在位置 $x_n$ から次の $x_{n+1}$ への $1$ 歩だけを
見積もればよい。
ところがモデルはこの観測済みの途中経過を予測に使わず，
$60$ 歩すべてをまとめて確率的な平均で置き換えてしまう。
その結果，いま値がどこにあるか（現在位置 $x_n$）も，
直前がどちら向きに動いたかも，予測に反映されない。
言いかえれば，予測は「いまの状況を手がかりにしない平均値」になっている。

これは，直近の実測値を出発点として次の $1$ 歩だけを見積もる場合と比べ，
見積もりの対象となる区間そのものが長いことを意味する。
実際の値動きは毎回この平均のまわりに散らばるので，
出発点が遠く，見積もる歩数が多いほど，実際の経路が平均からずれる余地は大きくなる
（ずれの幅は，見る歩数のおよそ平方根に比例して増える）。
したがって，$W$ が大きいほど MAE が増加するという結果（表\,\ref{tab:w_sweep}）は，
当てはめのばらつきといった偶然の要因よりも前に，
\textbf{遠い過去を起点として長い区間を見積もる}という
予測の作りそのものから生じる，ある程度避けられない性質であると考えられる。
