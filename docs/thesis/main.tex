...
各区間の MAE と方向的中率 HIT を表\,\ref{tab:pattern_summary} にまとめる。
...
\begin{table}[H]\centering
  \caption{各パターン・各参照期間の MAE と方向的中率 HIT（合成データ，$N=150$）}
  \label{tab:pattern_summary}
  \begin{tabular}{l|ccc|ccc}\hline
    & \multicolumn{3}{c|}{MAE} & \multicolumn{3}{c}{HIT} \\
    パターン & $W=2$ & $W=10$ & $W=60$ & $W=2$ & $W=10$ & $W=60$ \\\hline
    上昇継続     & 0.996 & 1.407 & 4.247 & 0.503 & 0.443 & 0.369 \\
    レンジ→上昇 & 1.082 & 1.124 & 2.442 & 0.436 & 0.524 & 0.497 \\
    上昇→レンジ & 0.996 & 1.368 & 2.963 & 0.503 & 0.443 & 0.470 \\
    上昇→下降   & 0.892 & 1.485 & 4.876 & 0.584 & 0.463 & 0.403 \\\hline
  \end{tabular}
\end{table}

方向的中率 HIT は，MAE に比べてインスタンス間のばらつきが大きい。
本表の系列では全体として $0.5$ 前後にとどまり，
$0.5$ を明確に上回ったのは上昇→下降の $W=2$（$0.584$）のみであった。
別の乱数で生成した第 2 インスタンスでは，
MAE の順序（$W=2<W=10<W=60$）が 4 パターンのすべてで保たれた一方，
$W=2$ の HIT は $0.53$〜$0.60$ と本表より高めに出た。
すなわち $W=2$ の的中率は生成された系列の継続率をそのまま映す量であり，
方向の持続が強い系列でのみ $0.5$ を上回る。
また，MAE では基準値 $1$ を下回る $W=2$ でも方向的中率がコイン投げと
同程度にとどまる場合があることは，
小さい $W$ の予測が方向を確実に当てているのではなく，
予測の変化幅を小さく抑えることで外れたときの誤差を
小さくしていることを示している（この機構は第8章で述べる）。
...
方向的中率 HIT で安定して $0.5$ を上回る設定はなく，
$W=2$ が方向の持続の強い系列で $0.5$ を上回るにとどまった。
