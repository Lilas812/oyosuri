...
\begin{proposition}[時間発展対称な相関付きランダムウォークの対称性]
\label{prop:sym}
...
時刻 $n = 1000$ までの分布 $P(\tilde{S}_{n} = x)$ を厳密に計算した。
...
のいずれにおいても，$n = 1000$ という大きなステップ数では有限ステップ分布は
極限正規分布とほぼ完全に一致しており，定理\,\ref{thm:clt_corr} の弱収束が数値的に確認できる。
実際，スケーリング前の分散を数値的に評価すると，
$\operatorname{Var}(\tilde{S}_n)/n$ は持続性の場合で $1.9985\,(\approx 2)$，
反転傾向の場合で $0.5004\,(\approx 1/2)$ となり，
いずれも極限分散 $a/(1-a)$ とよく一致する。

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{crw_limit_simulation}
  \caption{時間発展対称な相関付きランダムウォークの有限ステップ分布（$n=1000$，厳密計算）と極限正規分布 $N(0,\,a/(1-a))$ の比較。左：持続性（$a=2/3$），右：反転傾向（$a=1/3$）。}
